...
std::unordered_map<std::string, AudioEngine::ProfileHelper>
    AudioEngine::_audioPathProfileHelperMap;
  ...
  AudioEngineThreadPool(int threads = 4) : _stop(false) {
    for (int index = 0; index < threads; ++index) {
      _workers.emplace_back(
          std::thread(std::bind(&AudioEngineThreadPool::threadFunc, this)));
    }
  }
...
void AudioEngine::end() {
  ...
  if (_onPauseListenerID != 0) {
    EventDispatcher::removeCustomEventListener(EVENT_COME_TO_BACKGROUND,
                                               _onPauseListenerID);
    _onPauseListenerID = 0;
  }

  if (_onResumeListenerID != 0) {
    EventDispatcher::removeCustomEventListener(EVENT_COME_TO_FOREGROUND,
                                               _onResumeListenerID);
...
bool AudioEngine::lazyInit() {
  if (_audioEngineImpl == nullptr) {
    ...
    _onPauseListenerID = EventDispatcher::addCustomEventListener(
        EVENT_COME_TO_BACKGROUND, AudioEngine::onEnterBackground);
    _onResumeListenerID = EventDispatcher::addCustomEventListener(
        EVENT_COME_TO_FOREGROUND, AudioEngine::onEnterForeground);
  }
...
int AudioEngine::play2d(const std::string &filePath, bool loop, float volume,
  ...
  do {
    ...
    if (profileHelper) {
      if (profileHelper->profile.maxInstances != 0 &&
          profileHelper->audioIDs.size() >=
              profileHelper->profile.maxInstances) {
        CC_LOG_INFO(
      ...
      if (profileHelper->profile.minDelay > TIME_DELAY_PRECISION) {
        ...
        auto delay =
            (float)std::chrono::duration_cast<std::chrono::microseconds>(
                currTime - profileHelper->lastPlayTime)
                .count() /
            1000000.0;
        if (profileHelper->lastPlayTime.time_since_epoch().count() != 0 &&
            delay <= profileHelper->profile.minDelay) {
          CC_LOG_INFO("Fail to play %s cause by limited minimum delay",
                      filePath.c_str());
          break;
        }
      }
...
void AudioEngine::uncache(const std::string &filePath) {
  ...
  if (audioIDsIter != _audioPathIDMap.end()) {
    //@Note: For safely iterating elements from the audioID list, we need to
    // copy the list
    // since 'AudioEngine::remove' may be invoked in '_audioEngineImpl->stop'
    // synchronously. If this happens, it will break the iteration, and crash
    // will appear on some devices.
...
float AudioEngine::getDuration(int audioID) {
  auto it = _audioIDInfoMap.find(audioID);
  if (it != _audioIDInfoMap.end() &&
      it->second.state != AudioState::INITIALIZING) {
...
void AudioEngine::setFinishCallback(
    int audioID,
...
bool AudioEngine::isLoop(int audioID) {
  ...
  CC_LOG_INFO("AudioEngine::isLoop-->The audio instance %d is non-existent",
              audioID);
  return false;
}
...
float AudioEngine::getVolume(int audioID) {
  ...
  CC_LOG_INFO("AudioEngine::getVolume-->The audio instance %d is non-existent",
              audioID);
  return 0.0f;
}
